export interface ConfigurationType {
  anonymousContributorId: string
  anonymousContributorName: string
  applicationName: string
  basePath: string
  baseUrl: string
  catalogRepositoryName: string
  defaultRepositoryId: string
  defaultRepositoryName: string
  homeId: string
  rowHeightInPixels: number
  VERSION: string
}

const Configuration = {
  /**
   * Unique ID for anonymous contributors (contributors who haven't had their identities verified).
   * @type {string}
   */
  anonymousContributorId: 'guest@sqwerl.com',

  /**
   * Name for anonymous contributors.
   * @type {string}
   */
  anonymousContributorName: 'guest',

  /**
   * The name of the server application that this client application communicates with. This client application
   * embeds this server application's name in the URLs it opens in order to communicate with the server.
   * @type {string}
   */
  applicationName: 'sqwerl',

  /**
   * The base path of the URL that points to this application.
   */
  basePath: '/',

  /**
   * The base URL of the first thing this client application displays. Typically, this is a path to the default
   * repository of things that all guest (anonymous) users are allowed to view.
   * @type {string}
   */
  baseUrl: '/sqwerl/Main',

  /**
   * The name of the default catalog of things. A catalog is a repository of things that describe users, user
   * accounts, and security settings. It contains things that are shared by multiple child repositories.
   * @type {string}
   */
  catalogRepositoryName: 'catalog',

  /**
   * The unique identifier for the repository of things that guest users view when they start this application.
   * @type {string}
   */
  defaultRepositoryId: '/types/repositories/Main',

  /**
   * The name of the default repository of things that guest (anonymous) users are allowed to view.
   * @type {string}
   */
  defaultRepositoryName: 'Main',

  /**
   * The ID of the first thing this application displays.
   * @type {string}
   */
  homeId: '/types/views/initial',

  /**
   * This is the default height, in pixels, for rows within infinite scrollers. This value needs to be in sync
   * with its corresponding CSS variable.
   */
  rowHeightInPixels: 80,

  /**
   * The version number of the protocol that this client application uses to communicate to its server application.
   * @type {string}
   */
  VERSION: '0.1.2'
}

export default Configuration
