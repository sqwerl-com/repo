...
/**
 * Aggregates the given changes made to a thing to the days that the changes were made on.
 * @param changes Recent changes made to a thing.
 */
export const aggregateToChangesByDay = (changes: ChangesShape[]): AggregatedChange[] => {
  const changesByDay: AggregatedChange[] = []
  if (changes) {
    let index = 0
