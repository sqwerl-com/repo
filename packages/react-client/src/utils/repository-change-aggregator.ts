import { RepositoryChangesShape } from '@/utils/types'

export interface AggregatedRepositoryChange {
  by: string[]
  changesCount: number
  date: string
...
/**
 * Aggregates the given changes to a repository of things to the days that the changes were made on.
 * @param changes Recent changes made to a repository of things.
 */
export const aggregateToChangesByDay = (changes: RepositoryChangesShape[]): AggregatedRepositoryChange[] => {
  const changesByDay: AggregatedRepositoryChange[] = []
  if (changes) {
    let index = 0
